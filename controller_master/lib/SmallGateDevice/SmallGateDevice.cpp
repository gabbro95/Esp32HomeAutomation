...
void SmallGateDevice::evaluateAutomationRules_SmallGateChanged(SmallGateActualState oldState, SmallGateActualState newState) {
    ...
    } else if (newState == SMALL_GATE_ACTUAL_CLOSED) {
...
void SmallGateDevice::loop() {
    ...
    if (!digitalRead(RELAY_PIN) && getState().smallGateActual == SMALL_GATE_ACTUAL_CLOSED && switchOffTimer.check() && switchOffTimer.isExpired()) {
        // Il timer è scaduto!
