...
void DisplayRusticoDevice::handleMessage(const EspNowMessage& msg) {
    ...
    if (finalDest == DEV_CENTRAL_MASTER) {
        ...
        if (finalDest != DEV_GARAGE) {
            ...
            // Caso specifico Garage (se gestito diversamente, o se è alias di Central)
            PeerDevice* target = peerManager->findPeerByDevice(static_cast<DeviceType>(DEV_CENTRAL));
            if (target) {
                peerManager->sendOrQueue(target->getMacAddress(), copy);
            }
        }
